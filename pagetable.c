...
static int allocate_frame(pt_entry_t *pte)
{
	...
	if (frame == -1) { // Didn't find a free page.
		...
		if(victim_pte->frame & PAGE_DIRTY){
			int swap_offset = swap_pageout(frame, victim_pte->swap_offset);
		...
		victim_pte->frame &= ~PAGE_VALID;
		victim_pte->frame |= PAGE_ONSWAP;
		victim_pte->frame &= ~PAGE_DIRTY;
	}
...
char *find_physpage(vaddr_t vaddr, char type)
{
	...
	pd_entry_t *slpt = (pd_entry_t *)(pdpt[idx].pdp & PAGE_MASK) + PT_INDEX(vaddr);

	// Use 2nd-level page table and vaddr to get pointer to 3rd-level page table
	if(slpt[idx].pde == 0){
		slpt[idx] = init_page_table();
	}
	uintptr_t tlpd = slpt[idx].pde;

	// Use 3rd-level page table and vaddr to get pointer to page table entry
	pte = (pt_entry_t *)(tlpd & PAGE_MASK) + PT_INDEX(vaddr);
	...
	if (pte->frame & PAGE_VALID){
		...
		if(pte->frame & PAGE_ONSWAP){
			int res = swap_pagein(frame, pte->swap_offset);
			if (res != 0){
				perror("Failed to swap pagein.");
				exit(1);
			}
